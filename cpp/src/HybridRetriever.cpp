...
std::vector<std::string> TokenizeAlnumLower(const std::string& text) {
...
std::vector<HybridRetriever::RetrievalResult> HybridRetriever::FuseResults(
    ...
    for (const auto& result : dense_results) {
        ...
        doc_map[result.doc_id].fused_score =
            effective_dense_weight * CalculateRRFScore(result.dense_rank, config.rrf_k);
    ...
    for (const auto& result : sparse_results) {
        if (doc_map.find(result.doc_id) != doc_map.end()) {
            ...
            doc_map[result.doc_id].fused_score +=
                effective_sparse_weight * CalculateRRFScore(result.sparse_rank, config.rrf_k);
            doc_map[result.doc_id].base_fused_score = doc_map[result.doc_id].fused_score;
    ...
    const double guard_lambda = GuardLambdaForState(config);
    const double risk_scale = Clamp01(config.injection_penalty_scale * guard_lambda);
    const double guard_bias = guard_lambda - 1.0;
    const double min_trust_required = Clamp01(config.min_trust_score + (0.08 * guard_bias));
    const double max_risk_allowed = Clamp01(config.max_injection_risk - (0.10 * guard_bias));

    for (const auto& [doc_id, result] : doc_map) {
        RetrievalResult adjusted = result;

        if (config.enable_robustness_guard) {
            if (adjusted.trust_score < min_trust_required ||
                adjusted.injection_risk > max_risk_allowed) {
                dropped_by_guard++;
                continue;
            }
...
std::vector<HybridRetriever::RetrievalResult> HybridRetriever::Search(
    ...
    // Fuse results using state-conditioned alpha(s) with optional lexical adaptation alpha(s, q).
    auto [effective_dense_weight, effective_sparse_weight] = ResolveFusionWeights(config);
    const auto query_adjusted_weights = ApplyQueryAwareFusion(
        query,
        sparse_results,
        config,
    ...
    effective_dense_weight = query_adjusted_weights.first;
    effective_sparse_weight = query_adjusted_weights.second;
    auto fused_results = FuseResults(
        dense_results,
        sparse_results,
        config,
        effective_dense_weight,
    ...
    std::cerr << "HybridRetriever: Query='" << query << "', "
              ...
              << "Fused=" << fused_results.size()
              << ", alpha_dense=" << effective_dense_weight
              << ", alpha_sparse=" << effective_sparse_weight
              << ", lexical_adapt=" << (config.query_aware_fusion_enabled ? "on" : "off")
              << std::endl;
...
double HybridRetriever::LexicalOverlapRatio(const std::string& query, const std::string& text) const {
    const auto query_tokens = TokenizeAlnumLower(query);
    const auto text_tokens = TokenizeAlnumLower(text);
    if (query_tokens.empty() || text_tokens.empty()) {
        return 0.0;
    }

    std::unordered_set<std::string> query_vocab(query_tokens.begin(), query_tokens.end());
    std::unordered_set<std::string> text_vocab(text_tokens.begin(), text_tokens.end());
    if (query_vocab.empty() || text_vocab.empty()) {
        return 0.0;
    }

    size_t hit = 0;
    for (const auto& tok : query_vocab) {
        if (text_vocab.find(tok) != text_vocab.end()) {
            hit++;
        }
    }
    return static_cast<double>(hit) / static_cast<double>(query_vocab.size());
}

std::pair<double, double> HybridRetriever::ApplyQueryAwareFusion(
    ...
    double dense = std::max(0.0, dense_weight);
    double sparse = std::max(0.0, sparse_weight);
    if (!config.query_aware_fusion_enabled || sparse_results.empty()) {
        const double sum = dense + sparse;
        if (sum <= 1e-6) {
            return {0.6, 0.4};
        }
        return {dense / sum, sparse / sum};
    }

    const double overlap = Clamp01(LexicalOverlapRatio(query, sparse_results.front().text));
    const double low = Clamp01(config.lexical_low_overlap_threshold);
    const double high = Clamp01(config.lexical_high_overlap_threshold);

    if (overlap <= low) {
        dense += std::max(0.0, config.lexical_dense_boost_low_overlap);
    } else if (overlap >= high) {
        sparse += std::max(0.0, config.lexical_sparse_boost_high_overlap);
    }
...
double HybridRetriever::GuardLambdaForState(const RetrievalConfig& config) const {
    if (!config.state_conditioned_guard_enabled) {
        return 1.0;
    }

    const std::string state = ToLower(config.guard_behavior_state);
    if (state.empty()) {
        return 1.0;
    }

    if (ContainsAny(state, {"guard", "detain", "investigat", "combat", "patrol", "alert"})) {
        return std::max(0.5, config.guard_lambda_conflict);
    }
    if (ContainsAny(state, {"assist", "quest", "handoff", "trade", "negotiat", "repair", "treat"})) {
        return std::max(0.5, config.guard_lambda_task);
    }
    if (ContainsAny(state, {"social", "idle", "chat", "greet", "observe"})) {
        return std::max(0.5, config.guard_lambda_social);
    }
    return 1.0;
}

std::pair<double, double> HybridRetriever::ResolveFusionWeights(const RetrievalConfig& config) const {
    double dense = std::max(0.0, config.dense_weight);
    double sparse = std::max(0.0, config.sparse_weight);
    const std::string state = ToLower(config.guard_behavior_state);

    if (config.state_conditioned_fusion_enabled && !state.empty()) {
        if (ContainsAny(state, {"guard", "detain", "investigat", "combat", "patrol", "alert"})) {
            dense = std::max(0.0, config.dense_weight_conflict);
            sparse = std::max(0.0, 1.0 - dense);
        } else if (ContainsAny(state, {"assist", "quest", "handoff", "trade", "negotiat", "repair", "treat"})) {
            dense = std::max(0.0, config.dense_weight_task);
            sparse = std::max(0.0, 1.0 - dense);
        } else if (ContainsAny(state, {"social", "idle", "chat", "greet", "observe"})) {
            dense = std::max(0.0, config.dense_weight_social);
            sparse = std::max(0.0, 1.0 - dense);
        }
    }

    const double denom = dense + sparse;
    if (denom <= 1e-6) {
        return {0.6, 0.4};
    }
    return {dense / denom, sparse / denom};
}
