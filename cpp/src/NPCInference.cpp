...
std::string ExtractBehaviorStateForGuard(const json& state) {
    if (!state.is_object()) {
        return "";
    }

    const std::vector<std::string> keys = {"behavior_state", "behaviortreestate", "behavior"};
    ...
    if (state.contains("current_state") && state["current_state"].is_object()) {
        const auto& current_state = state["current_state"];
        for (const auto& key : keys) {
            if (current_state.contains(key) && current_state[key].is_string()) {
                const std::string value = TrimCopy(current_state[key].get<std::string>());
                if (!value.empty()) {
                    return value;
                }
            }
...
namespace NPCInference {
    ...
    bool NPCInferenceEngine::SaveState(const std::string& filepath) {
        try {
            ...
            config_json["retrieval_guard_state_conditioned_enabled"] = config_.retrieval_guard_state_conditioned_enabled;
            config_json["retrieval_guard_lambda_conflict"] = config_.retrieval_guard_lambda_conflict;
            config_json["retrieval_guard_lambda_task"] = config_.retrieval_guard_lambda_task;
            config_json["retrieval_guard_lambda_social"] = config_.retrieval_guard_lambda_social;
            config_json["retrieval_state_conditioned_fusion_enabled"] = config_.retrieval_state_conditioned_fusion_enabled;
            config_json["retrieval_dense_weight_conflict"] = config_.retrieval_dense_weight_conflict;
            config_json["retrieval_dense_weight_task"] = config_.retrieval_dense_weight_task;
            config_json["retrieval_dense_weight_social"] = config_.retrieval_dense_weight_social;
            config_json["retrieval_query_aware_fusion_enabled"] = config_.retrieval_query_aware_fusion_enabled;
            config_json["retrieval_lexical_low_overlap_threshold"] = config_.retrieval_lexical_low_overlap_threshold;
            config_json["retrieval_lexical_high_overlap_threshold"] = config_.retrieval_lexical_high_overlap_threshold;
            config_json["retrieval_lexical_dense_boost_low_overlap"] = config_.retrieval_lexical_dense_boost_low_overlap;
            config_json["retrieval_lexical_sparse_boost_high_overlap"] = config_.retrieval_lexical_sparse_boost_high_overlap;
            ...
            config_json["response_control_state_conditioned_scoring_enabled"] =
                config_.response_control_state_conditioned_scoring_enabled;
            config_json["response_control_state_conditioned_acceptance_enabled"] =
                config_.response_control_state_conditioned_acceptance_enabled;
            config_json["response_control_low_risk_context_relax"] =
                config_.response_control_low_risk_context_relax;
            config_json["response_control_low_risk_persona_relax"] =
                config_.response_control_low_risk_persona_relax;
            config_json["response_control_low_risk_candidate_score_relax"] =
                config_.response_control_low_risk_candidate_score_relax;
            config_json["response_control_high_risk_context_tighten"] =
                config_.response_control_high_risk_context_tighten;
            config_json["response_control_high_risk_persona_tighten"] =
                config_.response_control_high_risk_persona_tighten;
            config_json["response_control_high_risk_candidate_score_tighten"] =
                config_.response_control_high_risk_candidate_score_tighten;
    ...
    std::string NPCInferenceEngine::GenerateWithState(const std::string& prompt, nlohmann::json& state, const std::string& last_thought, bool is_json) {
        ...
        auto apply_response_control = [&](const std::string& raw_output) -> std::string {
            ...
            rc_config.state_conditioned_scoring_enabled =
                config_.response_control_state_conditioned_scoring_enabled;
            rc_config.state_conditioned_acceptance_enabled =
                config_.response_control_state_conditioned_acceptance_enabled;
            rc_config.low_risk_context_relax = config_.response_control_low_risk_context_relax;
            rc_config.low_risk_persona_relax = config_.response_control_low_risk_persona_relax;
            rc_config.low_risk_candidate_score_relax =
                config_.response_control_low_risk_candidate_score_relax;
            rc_config.high_risk_context_tighten = config_.response_control_high_risk_context_tighten;
            rc_config.high_risk_persona_tighten = config_.response_control_high_risk_persona_tighten;
            rc_config.high_risk_candidate_score_tighten =
                config_.response_control_high_risk_candidate_score_tighten;
             ...
             if (config_.enable_rag && embedding_model_->IsLoaded()) {
                 ...
                 if (!query_vec.empty()) {
                      if (hybrid_retriever_) {
                          ...
                          retrieval_config.state_conditioned_guard_enabled = (
                              config_.retrieval_guard_state_conditioned_enabled
                          );
                          retrieval_config.guard_lambda_conflict = config_.retrieval_guard_lambda_conflict;
                          retrieval_config.guard_lambda_task = config_.retrieval_guard_lambda_task;
                          retrieval_config.guard_lambda_social = config_.retrieval_guard_lambda_social;
                          retrieval_config.state_conditioned_fusion_enabled =
                              config_.retrieval_state_conditioned_fusion_enabled;
                          retrieval_config.dense_weight_conflict = config_.retrieval_dense_weight_conflict;
                          retrieval_config.dense_weight_task = config_.retrieval_dense_weight_task;
                          retrieval_config.dense_weight_social = config_.retrieval_dense_weight_social;
                          retrieval_config.query_aware_fusion_enabled =
                              config_.retrieval_query_aware_fusion_enabled;
                          retrieval_config.lexical_low_overlap_threshold =
                              config_.retrieval_lexical_low_overlap_threshold;
                          retrieval_config.lexical_high_overlap_threshold =
                              config_.retrieval_lexical_high_overlap_threshold;
                          retrieval_config.lexical_dense_boost_low_overlap =
                              config_.retrieval_lexical_dense_boost_low_overlap;
                          retrieval_config.lexical_sparse_boost_high_overlap =
                              config_.retrieval_lexical_sparse_boost_high_overlap;
                          retrieval_config.guard_behavior_state = ExtractBehaviorStateForGuard(local_state);
