...
void TestVision(NPCInference::NPCInferenceEngine& engine) {
    ...
    // Real implementation should always return a concrete diagnostic/description string.
    if (!desc.empty()) {
        std::cout << "[PASS] Vision Path Active" << std::endl;
    } else {
        std::cout << "[WARN] Vision output empty" << std::endl;
    }
}
