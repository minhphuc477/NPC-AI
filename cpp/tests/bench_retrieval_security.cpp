...
static bool ExtractTopPrediction(const json& row, std::string* scenario_id, std::string* top1_doc_id) {
    if (!row.is_object()) {
        return false;
    }
    std::string sid = row.value("scenario_id", "");
    if (sid.empty()) {
        sid = row.value("query_id", "");
    }

    std::string top1 = row.value("top1_doc_id", "");
    if (top1.empty()) {
        top1 = row.value("doc_id", "");
    }
    if (top1.empty() && row.contains("predicted_doc_ids") && row["predicted_doc_ids"].is_array() && !row["predicted_doc_ids"].empty()) {
        const auto& first = row["predicted_doc_ids"][0];
        if (first.is_string()) {
            top1 = first.get<std::string>();
        }
    }
    if (top1.empty()) {
        const std::vector<std::string> candidates = {"ranked_results", "results", "top_docs", "ranked"};
        for (const auto& key : candidates) {
            if (!row.contains(key) || !row[key].is_array() || row[key].empty()) {
                continue;
            }
            const auto& first = row[key][0];
            if (!first.is_object()) {
                continue;
            }
            if (first.contains("doc_id") && first["doc_id"].is_string()) {
                top1 = first["doc_id"].get<std::string>();
                break;
            }
            if (first.contains("id") && first["id"].is_string()) {
                top1 = first["id"].get<std::string>();
                break;
            }
        }
    }

    if (sid.empty() || top1.empty()) {
        return false;
    }
    *scenario_id = sid;
    *top1_doc_id = top1;
...
static void AppendPredictionRows(
    const json& rows,
    ...
    for (const auto& row : rows) {
        std::string scenario_id;
        std::string top1_doc_id;
        if (ExtractTopPrediction(row, &scenario_id, &top1_doc_id)) {
            (*out)[scenario_id] = top1_doc_id;
        }
    }
}

static std::unordered_map<std::string, std::string> LoadExternalTop1Predictions(const std::string& path) {
    std::unordered_map<std::string, std::string> out;
    if (path.empty()) {
        return out;
    }
    ...
    if (!in.good()) {
        std::cerr << "[warn] car predictions file not found: " << path << std::endl;
        return out;
    }

    bool parsed_json_doc = false;
    if (path.size() >= 5 && path.substr(path.size() - 5) == ".json") {
        try {
            json doc = json::parse(in);
            parsed_json_doc = true;
            if (doc.is_array()) {
                AppendPredictionRows(doc, &out);
            } else if (doc.is_object()) {
                if (doc.contains("predictions")) {
                    AppendPredictionRows(doc["predictions"], &out);
                }
                if (doc.contains("per_scenario")) {
                    AppendPredictionRows(doc["per_scenario"], &out);
                }
                if (doc.contains("rows")) {
                    AppendPredictionRows(doc["rows"], &out);
                }
                if (doc.contains("results")) {
                    AppendPredictionRows(doc["results"], &out);
                }
            }
        } catch (...) {
            std::cerr << "[warn] failed to parse CAR predictions json: " << path << std::endl;
        }
    }
    ...
    if (!parsed_json_doc) {
        ...
        while (std::getline(in, line)) {
            ...
            try {
                ...
                std::cerr << "[warn] invalid CAR prediction row at line " << line_no << std::endl;
            }
        }
    }
...
int main(int argc, char* argv[]) {
    std::string output_path = "storage/artifacts/benchmarks/retrieval_security_results.json";
    std::string dataset_path = "data/retrieval_poison_benchmark.jsonl";
    std::string car_predictions_path;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--output" && i + 1 < argc) {
            ...
            dataset_path = argv[++i];
        } else if (arg == "--car-predictions" && i + 1 < argc) {
            car_predictions_path = argv[++i];
    ...
    const std::unordered_map<std::string, std::string> car_top1_by_scenario =
        LoadExternalTop1Predictions(car_predictions_path);
    ...
    int car_predicted_scenarios = 0;
    int car_attack_success = 0;
    int car_safe_top1 = 0;

    std::vector<int> baseline_hits;
    std::vector<int> guarded_hits;
    std::vector<int> guarded_safe_hits;
    std::vector<int> baseline_hits_on_car;
    std::vector<int> car_hits;
    std::vector<int> car_safe_hits;
    baseline_hits.reserve(scenarios.size());
    guarded_hits.reserve(scenarios.size());
    guarded_safe_hits.reserve(scenarios.size());
    baseline_hits_on_car.reserve(scenarios.size());
    car_hits.reserve(scenarios.size());
    car_safe_hits.reserve(scenarios.size());
    ...
    std::unordered_map<std::string, std::vector<int>> baseline_hits_on_car_by_attack;
    std::unordered_map<std::string, std::vector<int>> car_hits_by_attack;
    std::unordered_map<std::string, std::vector<int>> car_safe_hits_by_attack;
    ...
    for (const auto& s : scenarios) {
        ...
        const auto car_it = car_top1_by_scenario.find(s.scenario_id);
        const bool car_available = (car_it != car_top1_by_scenario.end());
        const std::string car_top1_doc_id = car_available ? car_it->second : "";
        const bool car_attack = car_available && (car_top1_doc_id == "poison_doc");
        const bool car_safe = car_available && (car_top1_doc_id == "safe_doc");
        ...
        if (car_available) {
            baseline_hits_on_car.push_back(baseline_attack ? 1 : 0);
            car_hits.push_back(car_attack ? 1 : 0);
            car_safe_hits.push_back(car_safe ? 1 : 0);
            baseline_hits_on_car_by_attack[attack_type].push_back(baseline_attack ? 1 : 0);
            car_hits_by_attack[attack_type].push_back(car_attack ? 1 : 0);
            car_safe_hits_by_attack[attack_type].push_back(car_safe ? 1 : 0);
            car_predicted_scenarios += 1;
            car_attack_success += car_attack ? 1 : 0;
            car_safe_top1 += car_safe ? 1 : 0;
        }
        ...
        per_scenario.push_back({
            ...
            {"car_top1_doc_id", car_available ? car_top1_doc_id : ""},
            {"car_prediction_available", car_available},
            {"baseline_attack_success", baseline_attack},
            {"guarded_attack_success", guarded_attack},
            {"guarded_safe_top1", guarded_safe},
            {"car_attack_success", car_available ? json(car_attack) : json(nullptr)},
            {"car_safe_top1", car_available ? json(car_safe) : json(nullptr)}
        });
    }
    ...
    const bool car_enabled = !car_predictions_path.empty();
    const bool car_has_coverage = car_predicted_scenarios > 0;
    const int car_n_int = car_predicted_scenarios;
    const double car_n = static_cast<double>(car_n_int);
    const int baseline_attack_success_on_car =
        std::accumulate(baseline_hits_on_car.begin(), baseline_hits_on_car.end(), 0);
    const double baseline_asr_on_car =
        car_n_int > 0 ? static_cast<double>(baseline_attack_success_on_car) / car_n : 0.0;
    const double car_asr = car_n_int > 0 ? static_cast<double>(car_attack_success) / car_n : 0.0;
    const double car_safe_rate = car_n_int > 0 ? static_cast<double>(car_safe_top1) / car_n : 0.0;
    const double car_asr_reduction_vs_baseline =
        baseline_asr_on_car > 0.0 ? (baseline_asr_on_car - car_asr) / baseline_asr_on_car : 0.0;
    const CI95 car_asr_ci = WilsonCI(car_attack_success, car_n_int);
    const CI95 car_safe_ci = WilsonCI(car_safe_top1, car_n_int);
    const CI95 car_asr_reduction_ci = BootstrapReductionCI(
        baseline_hits_on_car,
        car_hits,
        bootstrap_iters,
        seed + 31U
    );
    ...
    for (const auto& kv : baseline_hits_by_attack) {
        ...
        int car_count = 0;
        int car_attack_count = 0;
        int car_safe_count = 0;
        double car_rate = 0.0;
        double car_safe_rate_attack = 0.0;
        double car_reduction_vs_baseline = 0.0;
        CI95 car_rate_ci{0.0, 0.0};
        CI95 car_safe_ci_attack{0.0, 0.0};
        CI95 car_reduction_ci_attack{0.0, 0.0};
        if (car_has_coverage) {
            const auto c_it = car_hits_by_attack.find(attack_type);
            const auto cs_it = car_safe_hits_by_attack.find(attack_type);
            const auto cb_it = baseline_hits_on_car_by_attack.find(attack_type);
            if (c_it != car_hits_by_attack.end() && cs_it != car_safe_hits_by_attack.end() && cb_it != baseline_hits_on_car_by_attack.end()) {
                car_count = static_cast<int>(c_it->second.size());
                for (int hit : c_it->second) car_attack_count += hit;
                for (int hit : cs_it->second) car_safe_count += hit;
                if (car_count > 0) {
                    const double car_count_d = static_cast<double>(car_count);
                    car_rate = static_cast<double>(car_attack_count) / car_count_d;
                    car_safe_rate_attack = static_cast<double>(car_safe_count) / car_count_d;
                    int baseline_count_on_car_attack = 0;
                    for (int hit : cb_it->second) baseline_count_on_car_attack += hit;
                    const double baseline_rate_on_car_attack = static_cast<double>(baseline_count_on_car_attack) / car_count_d;
                    car_reduction_vs_baseline =
                        baseline_rate_on_car_attack > 0.0
                            ? (baseline_rate_on_car_attack - car_rate) / baseline_rate_on_car_attack
                            : 0.0;
                    car_rate_ci = WilsonCI(car_attack_count, car_count);
                    car_safe_ci_attack = WilsonCI(car_safe_count, car_count);
                    car_reduction_ci_attack = BootstrapReductionCI(
                        cb_it->second,
                        c_it->second,
                        bootstrap_iters,
                        seed + 2017U
        ...
        by_attack_type[attack_type] = {
            ...
            {"car_prediction_count", car_count},
            {"car_attack_success_count", car_attack_count},
            {"car_safe_top1_count", car_safe_count},
            {"car_attack_success_rate", car_rate},
            {"car_asr_ci95_low", car_rate_ci.low},
            {"car_asr_ci95_high", car_rate_ci.high},
            {"car_safe_top1_rate", car_safe_rate_attack},
            {"car_safe_top1_ci95_low", car_safe_ci_attack.low},
            {"car_safe_top1_ci95_high", car_safe_ci_attack.high},
            {"car_relative_asr_reduction_vs_baseline", car_reduction_vs_baseline},
            {"car_relative_asr_reduction_ci95_low", car_reduction_ci_attack.low},
            {"car_relative_asr_reduction_ci95_high", car_reduction_ci_attack.high}
        };
    }
    ...
    result["car_enabled"] = car_enabled;
    result["car_predictions_path"] = car_predictions_path;
    result["car_prediction_coverage_count"] = car_predicted_scenarios;
    ...
    result["car_attack_success_count"] = car_attack_success;
    result["car_safe_top1_count"] = car_safe_top1;
    ...
    result["baseline_attack_success_rate_on_car_coverage"] = baseline_asr_on_car;
    result["car_attack_success_rate"] = car_asr;
    result["car_asr_ci95_low"] = car_asr_ci.low;
    result["car_asr_ci95_high"] = car_asr_ci.high;
    result["car_safe_top1_rate"] = car_safe_rate;
    result["car_safe_top1_ci95_low"] = car_safe_ci.low;
    result["car_safe_top1_ci95_high"] = car_safe_ci.high;
    result["car_relative_asr_reduction_vs_baseline"] = car_asr_reduction_vs_baseline;
    result["car_relative_asr_reduction_ci95_low"] = car_asr_reduction_ci.low;
    result["car_relative_asr_reduction_ci95_high"] = car_asr_reduction_ci.high;
    ...
    if (car_enabled) {
        std::cout << "CAR predictions: " << car_predictions_path
                  << " (coverage " << car_predicted_scenarios << "/" << scenarios.size() << ")" << std::endl;
        if (!car_has_coverage) {
            std::cout << "CAR metrics skipped: no matching scenario_id entries found." << std::endl;
        }
    }
    ...
    if (car_enabled && car_has_coverage) {
        std::cout << "CAR ASR: " << car_asr * 100.0 << "% ["
                  << car_asr_ci.low * 100.0 << ", " << car_asr_ci.high * 100.0 << "]" << std::endl;
        std::cout << "CAR Safe@1: " << car_safe_rate * 100.0 << "% ["
                  << car_safe_ci.low * 100.0 << ", " << car_safe_ci.high * 100.0 << "]" << std::endl;
        std::cout << "CAR Relative ASR Reduction vs Baseline(coverage): "
                  << car_asr_reduction_vs_baseline * 100.0 << "% ["
                  << car_asr_reduction_ci.low * 100.0 << ", " << car_asr_reduction_ci.high * 100.0 << "]"
                  << std::endl;
    }
