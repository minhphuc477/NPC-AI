...
def render_report(
    ...
    bm25_count = retrieval_metrics.get("bm25", {}).get("query_count", "na")
    lines.append(f"- Labeled query count used for ablation: `{bm25_count}`")
    ...
    if security_metrics:
        ...
        if int(security_metrics.get("car_prediction_coverage_count", 0) or 0) > 0:
            lines.append(
                f"- CAR ASR: {security_metrics.get('car_attack_success_rate', float('nan')):.4f} "
                f"(95% CI: {security_metrics.get('car_asr_ci95_low', float('nan')):.4f}, "
                f"{security_metrics.get('car_asr_ci95_high', float('nan')):.4f})"
            )
            lines.append(
                f"- CAR Safe@1: {security_metrics.get('car_safe_top1_rate', float('nan')):.4f} "
                f"(95% CI: {security_metrics.get('car_safe_top1_ci95_low', float('nan')):.4f}, "
                f"{security_metrics.get('car_safe_top1_ci95_high', float('nan')):.4f})"
            )
            lines.append(
                f"- CAR relative ASR reduction vs baseline (coverage): "
                f"{security_metrics.get('car_relative_asr_reduction_vs_baseline', float('nan')):.4f} "
                f"(95% CI: {security_metrics.get('car_relative_asr_reduction_ci95_low', float('nan')):.4f}, "
                f"{security_metrics.get('car_relative_asr_reduction_ci95_high', float('nan')):.4f})"
            )
        lines.append(
    ...
    if security_spoofed_metrics:
        ...
        if int(security_spoofed_metrics.get("car_prediction_coverage_count", 0) or 0) > 0:
            lines.append(
                f"- CAR ASR: {security_spoofed_metrics.get('car_attack_success_rate', float('nan')):.4f} "
                f"(95% CI: {security_spoofed_metrics.get('car_asr_ci95_low', float('nan')):.4f}, "
                f"{security_spoofed_metrics.get('car_asr_ci95_high', float('nan')):.4f})"
            )
            lines.append(
                f"- CAR Safe@1: {security_spoofed_metrics.get('car_safe_top1_rate', float('nan')):.4f} "
                f"(95% CI: {security_spoofed_metrics.get('car_safe_top1_ci95_low', float('nan')):.4f}, "
                f"{security_spoofed_metrics.get('car_safe_top1_ci95_high', float('nan')):.4f})"
            )
            lines.append(
                f"- CAR relative ASR reduction vs baseline (coverage): "
                f"{security_spoofed_metrics.get('car_relative_asr_reduction_vs_baseline', float('nan')):.4f} "
                f"(95% CI: {security_spoofed_metrics.get('car_relative_asr_reduction_ci95_low', float('nan')):.4f}, "
                f"{security_spoofed_metrics.get('car_relative_asr_reduction_ci95_high', float('nan')):.4f})"
            )
        lines.append(
...
def main() -> None:
    ...
    parser.add_argument(
        "--car-checkpoint",
        default="",
        help="Optional CAR checkpoint dir; when set, adds CAR dense retrieval ablation arm.",
    )
    parser.add_argument(
        "--car-index",
        default="",
        help="Optional prebuilt CAR index .npz path (if empty, built in-memory from retrieval corpus).",
    )
    parser.add_argument(
        "--car-device",
        default="auto",
        choices=["auto", "cuda", "cpu"],
        help="Device used by CAR retriever.",
    )
    parser.add_argument(
        "--car-max-length",
        type=int,
        default=256,
        help="Max token length for CAR encoder.",
    )
    parser.add_argument(
        "--car-batch-size",
        type=int,
        default=64,
        help="Batch size for CAR encoding when building in-memory index.",
    )
    parser.add_argument(
        "--reranker-pairs",
    ...
    parser.add_argument(
        "--min-retrieval-queries",
        type=int,
        default=50,
        help="Minimum number of labeled retrieval queries required before ablation is allowed.",
    )
    ...
    parser.add_argument(
        "--output-root",
        default="storage/artifacts/publication",
    ...
    min_retrieval_queries = max(1, int(args.min_retrieval_queries))
    if len(unique_query_ids) < min_retrieval_queries:
        raise RuntimeError(
            "Retrieval ablation blocked: labeled query set too small "
            f"({len(unique_query_ids)} < required {min_retrieval_queries}). "
            "Regenerate/replace retrieval_gold with a larger labeled set."
        )
    ...
    run_config = {
        ...
        "hit_k": args.hit_k,
        "min_retrieval_queries": min_retrieval_queries,
        "seed": args.seed,
        ...
        "car_index": str(args.car_index),
        "car_device": str(args.car_device),
        "car_max_length": int(args.car_max_length),
        "car_batch_size": int(args.car_batch_size),
    ...
    car_stage: Dict[str, Any] | None = None
    car_retriever = None
    car_checkpoint = str(args.car_checkpoint).strip()
    car_index_path = str(args.car_index).strip()
    if car_checkpoint:
        try:
            from core.car_retriever import CARRetriever

            if car_index_path:
                if not Path(car_index_path).exists():
                    raise FileNotFoundError(f"CAR index not found: {car_index_path}")
                car_retriever = CARRetriever.from_checkpoint_and_index(
                    checkpoint=car_checkpoint,
                    index_path=car_index_path,
                    device=str(args.car_device),
                    max_length=int(args.car_max_length),
                )
            else:
                car_retriever = CARRetriever.from_checkpoint_and_corpus(
                    checkpoint=car_checkpoint,
                    corpus_rows=corpus,
                    device=str(args.car_device),
                    max_length=int(args.car_max_length),
                    batch_size=int(args.car_batch_size),
                )
            car_stage = {
                "ok": True,
                "checkpoint": car_checkpoint,
                ...
                "max_length": int(args.car_max_length),
                "batch_size": int(args.car_batch_size),
                "doc_count": len(getattr(car_retriever.index, "doc_ids", [])),
            }
        except Exception as exc:
            car_stage = {
                "ok": False,
    ...
    if car_stage is not None:
        write_json(retrieval_dir / "car_stage.json", car_stage)

    methods = ("bm25",) if args.skip_ablation_baselines else ("bm25", "keyword_overlap", "random")
    if car_retriever is not None:
        methods = tuple(list(methods) + ["car"])
    for method in methods:
        ...
        for item in gold:
            ...
            if method == "bm25":
                ...
                assert car_retriever is not None
                ranked = [doc_id for doc_id, _ in car_retriever.rank(query, args.hit_k)]
            else:
    ...
    if args.run_security_benchmark:
        ...
        car_predictions_for_security = retrieval_dir / "predictions_car.jsonl"
        if security_exe.exists():
            ...
            if car_predictions_for_security.exists():
                security_cmd.extend(["--car-predictions", str(car_predictions_for_security)])
            proc = subprocess.run(
                security_cmd,
                check=False,
                capture_output=True,
            ...
            if proc.returncode == 0 and security_out.exists():
                ...
                if args.run_security_spoofed_benchmark:
                    ...
                    if car_predictions_for_security.exists():
                        spoof_cmd.extend(["--car-predictions", str(car_predictions_for_security)])
    ...
    if car_stage is not None:
        print(f"  - {retrieval_dir / 'car_stage.json'}")
