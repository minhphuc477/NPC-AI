...
evalmod = importlib.import_module("run_proposal_alignment_eval")
...
def main() -> None:
    ...
    parser.add_argument("--enable-gspe-arm", action="store_true")
    parser.add_argument("--gspe-arm-id", default="proposed_contextual_gspe")
    parser.add_argument("--gspe-checkpoint-dir", default="")
    parser.add_argument("--gspe-base-model", default="microsoft/Phi-3-mini-4k-instruct")
    parser.add_argument("--gspe-device", default="auto", choices=["auto", "cuda", "cpu"])
    parser.add_argument("--gspe-load-in-4bit", action="store_true")
    parser.add_argument("--gspe-temperature", type=float, default=float("nan"))
    parser.add_argument("--gspe-max-tokens", type=int, default=0)
    parser.add_argument("--enable-episodic-memory", action="store_true")
    parser.add_argument("--episodic-memory-path", default="")
    parser.add_argument("--episodic-memory-arms", default="dynamic")
    parser.add_argument("--episodic-memory-top-k", type=int, default=3)
    parser.add_argument("--episodic-memory-min-score", type=float, default=0.12)
    parser.add_argument("--episodic-memory-max-records", type=int, default=4000)
    parser.add_argument("--enable-draft-then-score", action="store_true")
    parser.add_argument("--draft-max-tokens", type=int, default=20)
    parser.add_argument("--draft-max-resamples", type=int, default=1)
    parser.add_argument("--draft-score-threshold", type=float, default=0.36)
    parser.add_argument("--draft-temperature-jitter", type=float, default=0.08)
    parser.add_argument("--draft-timeout-s", type=int, default=30)
    parser.add_argument("--disable-draft-only-with-response-control", action="store_true")
    parser.add_argument(
    ...
    parser.add_argument("--disable-control-state-conditioned-scoring", action="store_true")
    ...
    parser.add_argument("--output-root", default="storage/artifacts/proposal")
    parser.add_argument(
        "--keep-batch-runs",
        action="store_true",
    ...
    for batch_idx, batch_rows in enumerate(batches):
        ...
        command = [
            ...
            str(args.timeout_s),
            "--gspe-arm-id",
            str(args.gspe_arm_id),
            "--gspe-checkpoint-dir",
            str(args.gspe_checkpoint_dir),
            "--gspe-base-model",
            str(args.gspe_base_model),
            "--gspe-device",
            str(args.gspe_device),
            "--gspe-temperature",
            str(args.gspe_temperature),
            "--gspe-max-tokens",
            str(args.gspe_max_tokens),
            "--episodic-memory-path",
            str(args.episodic_memory_path),
            "--episodic-memory-arms",
            str(args.episodic_memory_arms),
            "--episodic-memory-top-k",
            str(args.episodic_memory_top_k),
            "--episodic-memory-min-score",
            str(args.episodic_memory_min_score),
            "--episodic-memory-max-records",
            str(args.episodic_memory_max_records),
            "--draft-max-tokens",
            str(args.draft_max_tokens),
            "--draft-max-resamples",
            str(args.draft_max_resamples),
            "--draft-score-threshold",
            str(args.draft_score_threshold),
            "--draft-temperature-jitter",
            str(args.draft_temperature_jitter),
            "--draft-timeout-s",
            str(args.draft_timeout_s),
            "--min-arm-success-rate",
        ...
        maybe_append(command, "--enable-gspe-arm", bool(args.enable_gspe_arm))
        maybe_append(command, "--gspe-load-in-4bit", bool(args.gspe_load_in_4bit))
        ...
        maybe_append(command, "--enable-episodic-memory", bool(args.enable_episodic_memory))
        maybe_append(command, "--enable-draft-then-score", bool(args.enable_draft_then_score))
        maybe_append(
            command,
            "--disable-draft-only-with-response-control",
            bool(args.disable_draft_only_with_response_control),
        )
        ...
        maybe_append(
            ...
            bool(args.disable_control_state_conditioned_scoring),
        )
        maybe_append(
            command,
    ...
    if args.quality_gate:
        gate_command = [
            ...
            "--publication-root",
            str(Path("storage/artifacts/publication")),
        ]
