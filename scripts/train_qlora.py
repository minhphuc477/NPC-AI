...
class TrainingConfig:
    ...
    output_dir: str = "storage/outputs/adapter"
...
def detect_accelerator(requested: str) -> str:
    ...
    try:
        ...
        logger.debug("CUDA probe failed during accelerator detection: %s", exc)
    return "cpu"
...
def run_training(config: TrainingConfig, data_path: str) -> str:
    ...
    try:
        ...
        logger.debug("Could not patch importlib.find_spec for torchao: %s", exc)
    ...
    try:
        ...
        logger.debug("Could not apply transformers torchao compatibility patch: %s", exc)
    ...
    try:
        ...
        DataCollatorForLanguageModeling = getattr(transformers_mod, "DataCollatorForLanguageModeling")
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="storage/outputs/adapter", help="Output directory")
