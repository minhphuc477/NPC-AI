...
def compute_tfidf_similarity(hyps: List[str], refs: List[str]) -> Dict[str, Any]:
    ...
    pairwise_mod = importlib.import_module("sklearn.metrics.pairwise")
    TfidfVectorizer = getattr(text_mod, "TfidfVectorizer")
    cosine_similarity = getattr(pairwise_mod, "cosine_similarity")
...
def compute_bertscore(hyps: List[str], refs: List[str]) -> Dict[str, Any]:
    try:
        bert_mod = importlib.import_module("bert_score")
        bert_score = getattr(bert_mod, "score")
    except Exception:
    ...
    p_scores = [float(x) for x in P]
    r_scores = [float(x) for x in R]
    f1_scores = [float(x) for x in F1]
    return {
...
def run_evaluation(hyp_jsonl: str, ref_jsonl: Optional[str] = None, out_dir: str = "outputs_eval") -> Dict[str, Any]:
    ...
    states: List[str] = [str(h.get("npc_state")) for h in hyps if h.get("npc_state")]
    if states:
        fig, ax = plt.subplots(figsize=(4, 3))
        from collections import Counter

        c = Counter(states)
        labels = [str(k) for k in c.keys()]
