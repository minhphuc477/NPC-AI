...
def evaluate_pairwise(
    ...
    accuracy_score = getattr(metrics_mod, "accuracy_score")
    log_loss = getattr(metrics_mod, "log_loss")
    roc_auc_score = getattr(metrics_mod, "roc_auc_score")
...
def main() -> None:
    text_mod = importlib.import_module("sklearn.feature_extraction.text")
    linear_mod = importlib.import_module("sklearn.linear_model")
    joblib_mod = importlib.import_module("joblib")
    TfidfVectorizerCls = getattr(text_mod, "TfidfVectorizer")
    LogisticRegressionCls = getattr(linear_mod, "LogisticRegression")
    ...
    parser.add_argument("--output-dir", default="storage/artifacts/reranker/latest")
    parser.add_argument("--predictions-out", default="storage/artifacts/reranker/latest/predictions_reranker.jsonl")
    ...
    vectorizer = TfidfVectorizerCls(ngram_range=(1, 2), min_df=2, max_features=80000)
    x_train = vectorizer.fit_transform(train_texts)
    model = LogisticRegressionCls(
    ...
    joblib_mod.dump({"vectorizer": vectorizer, "model": model}, out_dir / "reranker_model.joblib")
