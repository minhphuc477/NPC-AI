...
try:
    from local_model_profiles import baseline_profile_choices, resolve_baseline_models
except ModuleNotFoundError:
    from scripts.local_model_profiles import baseline_profile_choices, resolve_baseline_models

TOKEN_RE = re.compile(r"[a-z0-9']+")
SENTENCE_SPLIT_RE = re.compile(r"(?<=[.!?])\s+")
MODEL_ID_RE = re.compile(r"[^a-z0-9]+")

DRAFT_META_MARKERS: Tuple[str, ...] = (
    "as an ai",
    "assistant",
    "system prompt",
    "your response here",
...
def control_config_payload(
    ...
    return {
        ...
        "state_conditioned_scoring_enabled": config.state_conditioned_scoring_enabled,
...
def build_prompt(
    row: Dict[str, Any],
    include_dynamic_context: bool,
    episodic_memories: Optional[Sequence[str]] = None,
) -> str:
...
def _parse_dynamic_context_map_normalized(dynamic_context: str) -> Dict[str, str]:
    out: Dict[str, str] = {}
    for chunk in [c.strip() for c in str(dynamic_context or "").split(";") if c.strip()]:
        if "=" in chunk:
            ...
            continue
        key_n = " ".join(str(key).strip().lower().replace("_", " ").split())
        value_n = " ".join(str(value).strip().split())
        if key_n and value_n and key_n not in out:
            out[key_n] = value_n
    return out


def resolve_episodic_memory_arm_set(raw: str, arms: Sequence[EvaluationArm]) -> set[str]:
    token = str(raw or "").strip().lower()
    if token in {"", "dynamic"}:
        return {arm.arm_id for arm in arms if arm.include_dynamic_context}
    if token in {"all", "*"}:
        return {arm.arm_id for arm in arms}
    if token in {"none", "off", "disable"}:
        return set()
    requested = {item.strip() for item in parse_list_arg(str(raw)) if item.strip()}
    available = {arm.arm_id for arm in arms}
    return {arm_id for arm_id in requested if arm_id in available}
...
def _draft_behavior_bucket(behavior_state: str) -> str:
    normalized = " ".join(str(behavior_state or "").strip().lower().replace("_", " ").split())
    if not normalized:
        return "general"
    if any(token in normalized for token in ("detain", "negotiat", "investigat", "combat", "guard", "deception")):
        return "conflict"
    if any(token in normalized for token in ("assist", "quest", "handoff", "craft", "trade", "repair", "treat", "research", "forg")):
        return "task"
    if any(token in normalized for token in ("patrol", "social", "idle", "chat", "observe")):
        return "social"
    return "general"


def draft_score_threshold_for_state(base_threshold: float, behavior_state: str) -> float:
    threshold = float(base_threshold)
    bucket = _draft_behavior_bucket(behavior_state)
    if bucket == "conflict":
        threshold += 0.08
    elif bucket == "task":
        threshold -= 0.02
        ...
        threshold -= 0.05
    return clamp01(max(0.15, min(0.95, threshold)))


def draft_candidate_score(
    response: str,
    *,
    persona: str,
    dynamic_context: str,
    ...
    cleaned = sanitize_npc_response(response or "")
    if not cleaned:
        return 0.0

    context_rel = context_relevance_score(cleaned, dynamic_context, context_keywords) if context_keywords else 0.0
    persona_cons = persona_consistency_score(cleaned, persona, persona_keywords)
    natural = naturalness_score(cleaned)
    token_count = len(tokenize(cleaned))
    length_norm = clamp01(float(token_count) / 20.0)

    score = 0.45 * context_rel + 0.30 * persona_cons + 0.20 * natural + 0.05 * length_norm
    lowered = cleaned.lower()

    penalty = 0.0
    if token_count < 5:
        penalty += 0.20
    if any(marker in lowered for marker in DRAFT_META_MARKERS):
        penalty += 0.22
    if context_keywords and keyword_coverage(cleaned, context_keywords) < 0.05:
        penalty += 0.08

    return clamp01(score - penalty)


def _draft_temperature_for_attempt(base_temp: float, jitter: float, attempt_idx: int) -> float:
    if attempt_idx <= 0:
        return float(base_temp)
    step = (attempt_idx + 1) // 2
    sign = 1.0 if (attempt_idx % 2 == 1) else -1.0
    return float(base_temp) + sign * float(jitter) * float(step)
...
def compute_optional_bertscore(
    ...
    try:
        ...
        setattr(iu, "_torchao_available", False)
        setattr(iu, "_torchao_version", "0.0.0")
    except Exception:
        print("torchao compatibility patch skipped for bertscore path.")
...
def parse_dynamic_context_map(dynamic_context: str) -> Dict[str, str]:
    return _parse_dynamic_context_map_normalized(dynamic_context)
...
def infer_alert_state_from_behavior(behavior_state: str) -> str:
    state = str(behavior_state or "").strip().lower()
    if any(token in state for token in ("detain", "combat")):
        return "combat"
    if any(token in state for token in ("investigat", "negotiat", "observ")):
        return "investigating"
    if any(token in state for token in ("idle", "recovery", "social")):
        return "idle"
    return "patrol"


def infer_suspicion_level_from_behavior(behavior_state: str) -> int:
    state = str(behavior_state or "").strip().lower()
    if any(token in state for token in ("detain", "combat")):
        return 4
    if any(token in state for token in ("investigat", "negotiat")):
        return 3
    if "observ" in state:
        return 2
    if any(token in state for token in ("craft", "idle", "recovery", "social")):
        return 0
    return 1


def build_gspe_game_state(
    *,
    scenario: Dict[str, Any],
    scenario_tags: Dict[str, str],
    dynamic_context: str,
) -> Dict[str, Any]:
    context = parse_dynamic_context_map(dynamic_context)
    behavior_state = (
        context.get("behavior_state")
        or context.get("behaviortreestate")
        or str(scenario_tags.get("behavior_state", "")).strip()
        or str(scenario.get("behavior_state", "")).strip()
        or "patrolling"
    )
    alert_state = (
        context.get("alert_state")
        or context.get("alertstate")
        or infer_alert_state_from_behavior(behavior_state)
    )
    location = (
        context.get("location")
        or str(scenario.get("location", "")).strip()
        or "unknown"
    )
    suspicion_raw = context.get("suspicion_level") or context.get("suspicionlevel")
    if suspicion_raw is None:
        suspicion_level = infer_suspicion_level_from_behavior(behavior_state)
    else:
        try:
            suspicion_level = int(float(suspicion_raw))
        except Exception:
            suspicion_level = infer_suspicion_level_from_behavior(behavior_state)
    suspicion_level = max(0, min(4, int(suspicion_level)))

    time_of_day = context.get("time_of_day") or context.get("timeofday") or "day"
    weather = context.get("weather") or "clear"
    recent_event = context.get("recent_event") or context.get("recentevent") or "none"
    nearby_threat = parse_bool_like(
        context.get("nearby_threat", context.get("nearbythreat", "")),
        default=any(token in str(behavior_state).lower() for token in ("detain", "combat")),
    )
    active_quest = parse_bool_like(
        context.get("active_quest", context.get("activequest", "")),
        default=any(token in str(behavior_state).lower() for token in ("quest", "handoff", "investigat")),
    )

    return {
        "alert_state": str(alert_state),
        "location": str(location),
        "behavior_state": str(behavior_state),
        "suspicion_level": int(suspicion_level),
        "time_of_day": str(time_of_day),
        "weather": str(weather),
        "nearby_threat": bool(nearby_threat),
        "active_quest": bool(active_quest),
        "recent_event": str(recent_event),
    }
...
def summarize_operational_metrics(
    ...
    def latency_summary_ms(values: Sequence[float]) -> Dict[str, float]:
        cleaned = [float(v) for v in values if isinstance(v, (int, float)) and float(v) >= 0.0]
        if not cleaned:
            return {
                "n": 0,
                "mean": float("nan"),
        ...
        return {
            ...
            "p90": float(percentile(cleaned, 0.90)),
            "p95": float(percentile(cleaned, 0.95)),
        }

    def retry_distribution(retry_attempts: Sequence[int], total: int) -> Dict[str, Any]:
        counts: Dict[int, int] = {}
        for raw in retry_attempts:
            val = int(max(0, int(raw)))
            counts[val] = int(counts.get(val, 0)) + 1
        ordered = []
        for retry_count in sorted(counts.keys()):
            count = int(counts[retry_count])
            ordered.append(
                {
                    "retry_count": retry_count,
                    "count": count,
                    "rate": (count / float(total)) if total > 0 else float("nan"),
                }
            )
        return {
            "total_requests": total,
            "counts": {str(k): int(v) for k, v in sorted(counts.items(), key=lambda kv: kv[0])},
            "rows": ordered,
        }

    def state_calibration(
        confidence_and_labels: Sequence[Tuple[float, int]],
        bins: int = 5,
    ) -> Dict[str, Any]:
        cleaned: List[Tuple[float, int]] = []
        for conf, label in confidence_and_labels:
            ...
            if c > 1.0:
                c = 1.0
            cleaned.append((c, int(1 if label else 0)))
        if not cleaned:
            return {
                "n": 0,
                "ece": float("nan"),
        ...
        n = len(cleaned)
        brier = float(statistics.fmean([(c - y) ** 2 for c, y in cleaned]))
        bin_rows: List[Dict[str, Any]] = []
        ece = 0.0
        for b in range(max(1, int(bins))):
            lo = b / float(bins)
            hi = (b + 1) / float(bins)
            if b == bins - 1:
                members = [(c, y) for c, y in cleaned if (c >= lo and c <= hi)]
            else:
                members = [(c, y) for c, y in cleaned if (c >= lo and c < hi)]
            if not members:
                continue
            conf_mean = float(statistics.fmean([c for c, _ in members]))
            acc = float(statistics.fmean([float(y) for _, y in members]))
            weight = len(members) / float(n)
    ...
    for arm_id, rows in responses_by_arm.items():
        ...
        if total == 0:
            output["arms"][arm_id] = {
                ...
                "latency_ms": latency_summary_ms([]),
                "raw_generation_latency_ms": latency_summary_ms([]),
                "rewrite_latency_ms": latency_summary_ms([]),
                "draft_prefilter_rate": float("nan"),
                "draft_prefilter_accepted_rate": float("nan"),
                "draft_prefilter_resample_rate": float("nan"),
                "draft_prefilter_rejection_rate": float("nan"),
                "draft_prefilter_avg_attempts": float("nan"),
                "draft_prefilter_latency_ms": latency_summary_ms([]),
                "episodic_memory_enabled_rate": float("nan"),
                "episodic_memory_hit_rate": float("nan"),
                "episodic_memory_avg_hits": float("nan"),
                "episodic_memory_write_rate": float("nan"),
                "retry_count_distribution": retry_distribution([], 0),
                "by_behavior_state": {},
            }
            continue
        ...
        calibration_pairs_all: List[Tuple[float, int]] = []
        calibration_by_behavior: Dict[str, List[Tuple[float, int]]] = {}
        source_counts: Dict[str, int] = {}
        retry_attempts_all: List[int] = []
        total_latency_ms_values: List[float] = []
        raw_latency_ms_values: List[float] = []
        rewrite_latency_ms_values: List[float] = []
        draft_prefilter_enabled_count = 0
        draft_prefilter_accepted_count = 0
        draft_prefilter_resample_count = 0
        draft_prefilter_attempts_total = 0
        draft_prefilter_rejected_attempts_total = 0
        draft_prefilter_latency_ms_values: List[float] = []
        episodic_enabled_count = 0
        episodic_hit_count = 0
        episodic_written_count = 0
        episodic_hits_total = 0
        ...
        for row in rows:
            ...
            context_cov = float(row.get("response_context_coverage", float("nan")))
            persona_cov = float(row.get("response_persona_coverage", float("nan")))
            if math.isnan(context_cov):
                context_cov = 0.0
            if math.isnan(persona_cov):
                persona_cov = 0.0
            confidence = max(0.0, min(1.0, 0.6 * context_cov + 0.4 * persona_cov))
            observed_success = int(
                bool(ok)
                and not bool(row.get("response_fallback"))
                and (str(row.get("response_control_source", "")) != "fallback")
            )
            calibration_pairs_all.append((confidence, observed_success))
            ...
            retry_attempts_all.append(attempts)
            ...
            raw_ns = float(row.get("total_duration_ns", 0) or 0)
            rewrite_ns = float(row.get("rewrite_total_duration_ns", 0) or 0)
            raw_ms = (raw_ns / 1e6) if raw_ns > 0.0 else float("nan")
            rewrite_ms = (rewrite_ns / 1e6) if rewrite_ns > 0.0 else 0.0
            if not math.isnan(raw_ms):
                raw_latency_ms_values.append(raw_ms)
                total_latency_ms_values.append(raw_ms + max(0.0, rewrite_ms))
            if rewrite_ns > 0.0:
                rewrite_latency_ms_values.append(rewrite_ms)

            if bool(row.get("draft_prefilter_enabled", False)):
                draft_prefilter_enabled_count += 1
                draft_attempts = int(row.get("draft_attempts", 0) or 0)
                draft_rejected_attempts = int(row.get("draft_rejected_attempts", 0) or 0)
                draft_prefilter_attempts_total += max(0, draft_attempts)
                draft_prefilter_rejected_attempts_total += max(
                    0,
                    min(draft_rejected_attempts, max(0, draft_attempts)),
                )
                if bool(row.get("draft_accepted", False)):
                    draft_prefilter_accepted_count += 1
                if draft_rejected_attempts > 0:
                    draft_prefilter_resample_count += 1
                draft_total_ns = float(row.get("draft_total_duration_ns", 0) or 0)
                if draft_total_ns > 0.0:
                    draft_prefilter_latency_ms_values.append(draft_total_ns / 1e6)

            if bool(row.get("episodic_memory_enabled", False)):
                episodic_enabled_count += 1
                hits = int(row.get("episodic_memory_retrieved", 0) or 0)
                episodic_hits_total += max(0, hits)
                if hits > 0:
                    episodic_hit_count += 1
                if bool(row.get("episodic_memory_written", False)):
                    episodic_written_count += 1
            ...
            calibration_by_behavior.setdefault(behavior, []).append((confidence, observed_success))

        by_behavior_rates: Dict[str, Any] = {}
        for behavior, counts in sorted(by_behavior.items()):
            denom = max(1, int(counts.get("count", 0)))
            behavior_cal = state_calibration(calibration_by_behavior.get(behavior, []), bins=5)
            by_behavior_rates[behavior] = {
                ...
                "calibration_ece": behavior_cal.get("ece", float("nan")),
                "calibration_brier": behavior_cal.get("brier", float("nan")),
            }

        overall_calibration = state_calibration(calibration_pairs_all, bins=10)
        ...
        output["arms"][arm_id] = {
            ...
            "latency_ms": latency_summary_ms(total_latency_ms_values),
            "raw_generation_latency_ms": latency_summary_ms(raw_latency_ms_values),
            "rewrite_latency_ms": latency_summary_ms(rewrite_latency_ms_values),
            "draft_prefilter_rate": (
                draft_prefilter_enabled_count / float(total)
            ),
            "draft_prefilter_accepted_rate": (
                draft_prefilter_accepted_count / float(draft_prefilter_enabled_count)
                if draft_prefilter_enabled_count > 0
                else float("nan")
            ),
            "draft_prefilter_resample_rate": (
                draft_prefilter_resample_count / float(draft_prefilter_enabled_count)
                if draft_prefilter_enabled_count > 0
                else float("nan")
            ),
            "draft_prefilter_rejection_rate": (
                draft_prefilter_rejected_attempts_total / float(draft_prefilter_attempts_total)
                if draft_prefilter_attempts_total > 0
                else float("nan")
            ),
            "draft_prefilter_avg_attempts": (
                draft_prefilter_attempts_total / float(draft_prefilter_enabled_count)
                if draft_prefilter_enabled_count > 0
                else float("nan")
            ),
            "draft_prefilter_latency_ms": latency_summary_ms(draft_prefilter_latency_ms_values),
            "episodic_memory_enabled_rate": episodic_enabled_count / float(total),
            "episodic_memory_hit_rate": (
                episodic_hit_count / float(episodic_enabled_count)
                if episodic_enabled_count > 0
                else float("nan")
            ),
            "episodic_memory_avg_hits": (
                episodic_hits_total / float(episodic_enabled_count)
                if episodic_enabled_count > 0
                else float("nan")
            ),
            "episodic_memory_write_rate": (
                episodic_written_count / float(episodic_enabled_count)
                if episodic_enabled_count > 0
                else float("nan")
            ),
            "state_calibration": overall_calibration,
            "retry_count_distribution": retry_distribution(retry_attempts_all, total),
...
def render_report(
    ...
    def safe_diff(lhs: Any, rhs: Any) -> float:
        try:
    ...
    for arm in arms:
        lines.append(
            ...
            f"backend=`{arm.generation_backend}`"
        )
    lines.append("")
    ...
    controlled_arm_id = "proposed_contextual_controlled"
    available_arm_ids = {arm.arm_id for arm in arms}
    gspe_arms = [arm for arm in arms if arm.generation_backend == "gspe_local_hf"]
    if gspe_arms and controlled_arm_id in available_arm_ids:
        lines.append("## GSPE vs Controlled")
        lines.append(
            "Direct comparison of GSPE backend arms against the default controlled arm "
            f"(`{controlled_arm_id}`)."
        )
        lines.append("")
        for gspe_arm in gspe_arms:
            comparison_key = f"{gspe_arm.arm_id}_vs_{controlled_arm_id}"
            if comparison_key not in deltas:
                lines.append(f"- `{gspe_arm.arm_id}`: comparison key `{comparison_key}` not found in delta outputs.")
                continue

            delta_metrics = deltas.get(comparison_key, {})
            paired_metrics = paired_deltas.get(comparison_key, {}) if paired_deltas else {}
            win_metrics = win_rates.get(comparison_key, {}) if win_rates else {}

            lines.append(f"### `{gspe_arm.arm_id}` vs `{controlled_arm_id}`")
            lines.append("| Metric | Delta | Paired Mean Delta (95% CI) | p(delta<=0) | Strict Non-tie Win Rate |")
            lines.append("|---|---:|---:|---:|---:|")
            for metric_name in sorted(delta_metrics.keys()):
                delta_vals = delta_metrics.get(metric_name, {})
                paired_vals = paired_metrics.get(metric_name, {})
                strict_vals = win_metrics.get(metric_name, {}).get("strict_non_tie_win_rate", {})
                lines.append(
                    f"| {metric_name} | "
                    f"{fmt_num(delta_vals.get('absolute_delta', float('nan')))} | "
                    f"{fmt_num(paired_vals.get('mean_delta', float('nan')))} "
                    f"({fmt_num(paired_vals.get('ci95_low', float('nan')))}, {fmt_num(paired_vals.get('ci95_high', float('nan')))}) | "
                    f"{fmt_num(paired_vals.get('p_delta_le_0', float('nan')))} | "
                    f"{fmt_num(strict_vals.get('mean', float('nan')))} |"
                )
            lines.append("")

            if operational_metrics:
                gspe_ops = operational_metrics.get("arms", {}).get(gspe_arm.arm_id, {})
                ctrl_ops = operational_metrics.get("arms", {}).get(controlled_arm_id, {})
                if isinstance(gspe_ops, dict) and isinstance(ctrl_ops, dict) and gspe_ops and ctrl_ops:
                    lines.append(
                        "- Operational deltas "
                        f"(GSPE - Controlled): first_pass_accept_rate={fmt_num(safe_diff(gspe_ops.get('first_pass_accept_rate'), ctrl_ops.get('first_pass_accept_rate')))}, "
                        f"retry_rate={fmt_num(safe_diff(gspe_ops.get('retry_rate'), ctrl_ops.get('retry_rate')))}, "
                        f"fallback_rate={fmt_num(safe_diff(gspe_ops.get('fallback_rate'), ctrl_ops.get('fallback_rate')))}."
                    )
                    lines.append("")
    ...
    if operational_metrics:
        ...
        lines.append(
            "| Arm | Timeout Rate | Error Rate | Fallback Rate | First-pass Accept Rate | Retry Rate | Latency p90 (ms) | Latency p95 (ms) |"
        )
        lines.append("|---|---:|---:|---:|---:|---:|---:|---:|")
        for arm in arms:
            ...
            lines.append(
                ...
                f"{latency.get('p90', float('nan')):.2f} | "
                f"{latency.get('p95', float('nan')):.2f} |"
            )
        lines.append("")
        lines.append("Per-retry-count distribution (share of requests):")
        lines.append("| Arm | Retry Distribution |")
        lines.append("|---|---|")
        for arm in arms:
            payload = operational_metrics.get("arms", {}).get(arm.arm_id, {})
            dist_rows = payload.get("retry_count_distribution", {}).get("rows", [])
            if isinstance(dist_rows, list) and dist_rows:
                parts = []
                for item in dist_rows:
                    retry_count = int(item.get("retry_count", 0))
                    count = int(item.get("count", 0))
                    rate = float(item.get("rate", float("nan")))
                    parts.append(f"{retry_count}:{count} ({rate:.3f})")
                dist_text = ", ".join(parts)
            else:
                dist_text = "n/a"
            lines.append(f"| {arm.arm_id} | {dist_text} |")
        lines.append("")
        lines.append("State-calibration diagnostics:")
        lines.append("| Arm | Calibration ECE | Calibration Brier |")
        lines.append("|---|---:|---:|")
        for arm in arms:
            payload = operational_metrics.get("arms", {}).get(arm.arm_id, {})
            cal = payload.get("state_calibration", {}) if isinstance(payload, dict) else {}
            lines.append(
                f"| {arm.arm_id} | {cal.get('ece', float('nan')):.4f} | {cal.get('brier', float('nan')):.4f} |"
            )
        lines.append("")
        if any(
            float(operational_metrics.get("arms", {}).get(arm.arm_id, {}).get("draft_prefilter_rate", 0.0) or 0.0) > 0.0
            for arm in arms
        ):
            lines.append("Draft-then-score prefilter diagnostics:")
            lines.append(
                "| Arm | Prefilter Rate | Accepted Rate | Resample Rate | Avg Attempts | Draft p95 (ms) |"
            )
            lines.append("|---|---:|---:|---:|---:|---:|")
            for arm in arms:
                payload = operational_metrics.get("arms", {}).get(arm.arm_id, {})
                draft_latency = payload.get("draft_prefilter_latency_ms", {})
                lines.append(
                    f"| {arm.arm_id} | "
                    f"{payload.get('draft_prefilter_rate', float('nan')):.4f} | "
                    f"{payload.get('draft_prefilter_accepted_rate', float('nan')):.4f} | "
                    f"{payload.get('draft_prefilter_resample_rate', float('nan')):.4f} | "
                    f"{payload.get('draft_prefilter_avg_attempts', float('nan')):.2f} | "
                    f"{draft_latency.get('p95', float('nan')):.2f} |"
                )
            lines.append("")
        if any(
            float(operational_metrics.get("arms", {}).get(arm.arm_id, {}).get("episodic_memory_enabled_rate", 0.0) or 0.0)
            > 0.0
            for arm in arms
        ):
            lines.append("Episodic memory diagnostics:")
            lines.append("| Arm | Enabled Rate | Hit Rate | Avg Retrieved Memories | Write Rate |")
            lines.append("|---|---:|---:|---:|---:|")
            for arm in arms:
                payload = operational_metrics.get("arms", {}).get(arm.arm_id, {})
                lines.append(
                    f"| {arm.arm_id} | "
                    f"{payload.get('episodic_memory_enabled_rate', float('nan')):.4f} | "
                    f"{payload.get('episodic_memory_hit_rate', float('nan')):.4f} | "
                    f"{payload.get('episodic_memory_avg_hits', float('nan')):.2f} | "
                    f"{payload.get('episodic_memory_write_rate', float('nan')):.4f} |"
                )
            lines.append("")
...
def main() -> None:
    ...
    parser.add_argument(
        "--enable-gspe-arm",
        action="store_true",
        help="Enable GSPE local-HF evaluation arm (non-Ollama backend).",
    )
    parser.add_argument(
        "--gspe-arm-id",
        default="proposed_contextual_gspe",
        help="Arm ID for GSPE local-HF evaluation.",
    )
    parser.add_argument(
        "--gspe-checkpoint-dir",
        default="",
        help="Checkpoint dir containing GSPE artifacts (gspe.pt + gspe_config.json).",
    )
    parser.add_argument(
        "--gspe-base-model",
        default="microsoft/Phi-3-mini-4k-instruct",
        help="Base HF model id/path for GSPE local-HF evaluation.",
    )
    parser.add_argument(
        "--gspe-device",
        default="auto",
        choices=["auto", "cuda", "cpu"],
        help="Device for GSPE local-HF inference arm.",
    )
    parser.add_argument(
        "--gspe-load-in-4bit",
        action="store_true",
        help="Load GSPE base model in 4-bit when supported.",
    )
    parser.add_argument(
        "--gspe-temperature",
        type=float,
        default=float("nan"),
        help="Optional GSPE arm temperature override (defaults to --temperature).",
    )
    parser.add_argument(
        "--gspe-max-tokens",
        type=int,
        default=0,
        help="Optional GSPE arm max token override (0 uses --max-tokens).",
    )
    parser.add_argument(
        "--enable-episodic-memory",
        action="store_true",
        help="Enable persistent episodic memory retrieval/injection during generation.",
    )
    parser.add_argument(
        "--episodic-memory-path",
        default="",
        help=(
            "Optional JSONL path for episodic memory store. "
            "If empty, stores are created under <run_dir>/episodic_memory/<arm_id>.jsonl."
        ),
    )
    parser.add_argument(
        "--episodic-memory-arms",
        default="dynamic",
        help=(
            "Arm selection for episodic memory: 'dynamic' (default), 'all', 'none', "
            "or comma-separated arm IDs."
        ),
    )
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Max retrieved episodic memories injected per prompt.",
    )
    parser.add_argument(
        "--episodic-memory-min-score",
        type=float,
        default=0.12,
        help="Minimum retrieval score for episodic memory injection.",
    )
    parser.add_argument(
        "--episodic-memory-max-records",
        type=int,
        default=4000,
        help="Maximum retained episodic memories per store.",
    )
    parser.add_argument(
        "--enable-draft-then-score",
        action="store_true",
        help="Enable lightweight draft generation + heuristic scoring before full generation.",
    )
    parser.add_argument(
        "--draft-max-tokens",
        type=int,
        default=20,
        help="Number of tokens to generate for each draft prefilter attempt.",
    )
    parser.add_argument(
        "--draft-max-resamples",
        type=int,
        default=1,
        help="Maximum number of additional draft resamples before full generation.",
    )
    parser.add_argument(
        "--draft-score-threshold",
        type=float,
        default=0.36,
        help="Base draft acceptance threshold before state-conditioned adjustment.",
    )
    parser.add_argument(
        "--draft-temperature-jitter",
        type=float,
        default=0.08,
        help="Temperature delta per draft resample attempt.",
    )
    parser.add_argument(
        "--draft-timeout-s",
        type=int,
        default=30,
        help="Timeout (seconds) for each draft prefilter request.",
    )
    parser.add_argument(
        "--disable-draft-only-with-response-control",
        action="store_true",
        help="Allow draft prefilter on all arms (default: only arms using response control).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--disable-control-state-conditioned-scoring",
        action="store_true",
        help="Disable state-conditioned candidate score weights and use fixed scoring weights.",
    )
    parser.add_argument(
    ...
    parser.add_argument("--output-root", default="storage/artifacts/proposal", help="Output root folder")
    ...
    extra_baselines = resolve_baseline_models(str(args.baseline_models), str(args.baseline_profile))
    ...
    base_control_config = ControlConfig(
        ...
        state_conditioned_scoring_enabled=not bool(args.disable_control_state_conditioned_scoring),
    )
    ...
    gspe_arm_id = str(args.gspe_arm_id).strip() or "proposed_contextual_gspe"
    if bool(args.enable_gspe_arm):
        if gspe_arm_id in {"proposed_contextual", "candidate_no_context", "proposed_contextual_controlled"}:
            raise ValueError(f"gspe-arm-id collides with reserved arm id: {gspe_arm_id}")
        checkpoint_dir = Path(str(args.gspe_checkpoint_dir).strip())
        if not checkpoint_dir.exists():
            raise FileNotFoundError(f"GSPE checkpoint dir not found: {checkpoint_dir}")
        arms.insert(
            1,
            EvaluationArm(
                arm_id=gspe_arm_id,
                model=str(args.gspe_base_model),
                include_dynamic_context=True,
                use_response_control=False,
                control_profile="none",
                generation_backend="gspe_local_hf",
            ),
        )
    ...
    run_config = {
        ...
        "gspe": {
            "enabled": bool(args.enable_gspe_arm),
            "arm_id": gspe_arm_id if bool(args.enable_gspe_arm) else "",
            "checkpoint_dir": str(args.gspe_checkpoint_dir),
            "base_model": str(args.gspe_base_model),
            "device": str(args.gspe_device),
            "load_in_4bit": bool(args.gspe_load_in_4bit),
            "temperature": (
                float(args.gspe_temperature)
                if not math.isnan(float(args.gspe_temperature))
                else float(args.temperature)
            ),
            "max_tokens": (
                int(args.gspe_max_tokens)
                if int(args.gspe_max_tokens) > 0
                else int(args.max_tokens)
            ),
        },
        "episodic_memory": {
            "enabled": bool(args.enable_episodic_memory),
            "path": str(args.episodic_memory_path),
            "arms": str(args.episodic_memory_arms),
            "top_k": max(1, int(args.episodic_memory_top_k)),
            "min_score": clamp01(float(args.episodic_memory_min_score)),
            "max_records": max(100, int(args.episodic_memory_max_records)),
        },
        "draft_then_score": {
            "enabled": bool(args.enable_draft_then_score),
            "only_with_response_control": not bool(args.disable_draft_only_with_response_control),
            "draft_max_tokens": max(1, int(args.draft_max_tokens)),
            "draft_max_resamples": max(0, int(args.draft_max_resamples)),
            "draft_score_threshold": clamp01(float(args.draft_score_threshold)),
            "draft_temperature_jitter": max(0.0, float(args.draft_temperature_jitter)),
            "draft_timeout_s": max(1, int(args.draft_timeout_s)),
        },
        ...
        "arms": [
            {
                ...
                "generation_backend": arm.generation_backend,
            }
            for arm in arms
        ],
    ...
    for arm in arms:
        model_name = str(arm.model)
        if model_name in model_meta:
            continue
        if arm.generation_backend == "ollama":
            model_meta[model_name] = query_ollama_model(args.host, model_name)
        else:
            model_meta[model_name] = {
                "model_name": model_name,
                "backend": arm.generation_backend,
                "host": "",
                "note": "local_hf_backend",
            }
    write_json(metadata_dir / "models.json", model_meta)

    episodic_store_by_arm: Dict[str, EpisodicMemoryStore] = {}
    episodic_stats_manifest: Dict[str, Any] = {"enabled": bool(args.enable_episodic_memory), "stores": {}}
    if bool(args.enable_episodic_memory):
        enabled_arm_ids = resolve_episodic_memory_arm_set(str(args.episodic_memory_arms), arms)
        episodic_top_k = max(1, int(args.episodic_memory_top_k))
        episodic_min_score = clamp01(float(args.episodic_memory_min_score))
        episodic_max_records = max(100, int(args.episodic_memory_max_records))

        provided_path = str(args.episodic_memory_path or "").strip()
        if provided_path:
            base_path = Path(provided_path)
            if len(enabled_arm_ids) <= 1:
                path_by_arm = {arm_id: base_path for arm_id in enabled_arm_ids}
            else:
                suffix = base_path.suffix or ".jsonl"
                stem = base_path.stem if base_path.suffix else base_path.name
                parent = base_path.parent if base_path.suffix else base_path
                path_by_arm = {
                    arm_id: parent / f"{stem}_{arm_id}{suffix}"
                    for arm_id in enabled_arm_ids
                }
        else:
            episodic_dir = run_dir / "episodic_memory"
            path_by_arm = {arm_id: episodic_dir / f"{arm_id}.jsonl" for arm_id in enabled_arm_ids}

        for arm in arms:
            if arm.arm_id not in enabled_arm_ids:
                continue
            store_path = path_by_arm[arm.arm_id]
            store = EpisodicMemoryStore(path=store_path, max_records=episodic_max_records)
            store.load()
            episodic_store_by_arm[arm.arm_id] = store
            episodic_stats_manifest["stores"][arm.arm_id] = {
                "path": str(store_path),
                "top_k": episodic_top_k,
                "min_score": episodic_min_score,
                "max_records": episodic_max_records,
                "initial_count": int(store.stats().get("count", 0)),
            }
        run_config["episodic_memory"]["resolved_arm_ids"] = sorted(enabled_arm_ids)
        run_config["episodic_memory"]["stores"] = episodic_stats_manifest["stores"]
        write_json(run_dir / "run_config.json", run_config)

    gspe_engine_by_arm: Dict[str, Any] = {}
    if bool(args.enable_gspe_arm):
        try:
            from gspe.gspe_inference import GenerationConfig as GSPEGenerationConfig
            from gspe.gspe_inference import GSPEInferenceEngine
        except Exception as exc:
            raise RuntimeError(
                "GSPE arm requested but GSPE runtime imports failed. "
                "Install required local-HF dependencies and retry."
            ) from exc

        gspe_checkpoint_dir = Path(str(args.gspe_checkpoint_dir).strip())
        gspe_temp = float(args.gspe_temperature)
        if math.isnan(gspe_temp):
            gspe_temp = float(args.temperature)
        gspe_max_tokens = int(args.gspe_max_tokens) if int(args.gspe_max_tokens) > 0 else int(args.max_tokens)
        gspe_engine = GSPEInferenceEngine.from_checkpoint(
            checkpoint_dir=str(gspe_checkpoint_dir),
            base_model=str(args.gspe_base_model),
            device=str(args.gspe_device),
            load_in_4bit=bool(args.gspe_load_in_4bit),
        )
        gspe_engine.generation_config = GSPEGenerationConfig(
            max_new_tokens=max(1, int(gspe_max_tokens)),
            temperature=float(gspe_temp),
            do_sample=True,
            repetition_penalty=1.05,
        )
        gspe_engine_by_arm[gspe_arm_id] = gspe_engine
    ...
    for arm in arms:
        print(
            ...
            f"response_control={arm.use_response_control} profile={arm.control_profile} backend={arm.generation_backend}"
        )
        for repeat_idx in range(max(1, args.repeats)):
            for scenario in scenarios:
                ...
                context_map = parse_dynamic_context_map(scenario_context)
                location = (
                    str(context_map.get("location", "")).strip()
                    or str(scenario_tags.get("location_type", "")).strip()
                    or str(scenario.get("location", "")).strip()
                )

                episodic_memories: List[Dict[str, Any]] = []
                episodic_lines: List[str] = []
                episodic_store = episodic_store_by_arm.get(arm.arm_id)
                episodic_top_k = max(1, int(args.episodic_memory_top_k))
                episodic_min_score = clamp01(float(args.episodic_memory_min_score))
                if episodic_store is not None:
                    ...
                    episodic_memories = episodic_store.retrieve(
                        query=query_text,
                        top_k=episodic_top_k,
                        min_score=episodic_min_score,
                        npc_id=scenario_persona,
                        behavior_state=behavior_state,
                    )
                    if episodic_memories:
                        episodic_formatted = format_episodic_memories(
                            episodic_memories,
                            max_items=episodic_top_k,
                        )
                        episodic_lines = [line.strip("- ").strip() for line in episodic_formatted.splitlines() if line.strip()]

                prompt = build_prompt(
                    scenario,
                    include_dynamic_context=arm.include_dynamic_context,
                    episodic_memories=episodic_lines,
                )

                draft_meta: Dict[str, Any] = {}
                generation_temperature = float(args.temperature)
                draft_enabled_for_arm = (arm.generation_backend == "ollama") and bool(args.enable_draft_then_score) and (
                    arm.use_response_control or bool(args.disable_draft_only_with_response_control)
                )
                if draft_enabled_for_arm:
                    draft_attempts_allowed = max(1, int(args.draft_max_resamples) + 1)
                    draft_threshold = draft_score_threshold_for_state(
                        float(args.draft_score_threshold),
                        behavior_state,
                    )
                    draft_total_duration_ns = 0
                    draft_total_eval_count = 0
                    draft_rejected_attempts = 0
                    draft_errors = 0
                    draft_accepted = False
                    draft_last_error = ""
                    draft_last_score = float("nan")
                    draft_best_score = float("nan")
                    selected_attempt = 0
                    accepted_attempt = 0

                    for attempt_idx in range(draft_attempts_allowed):
                        draft_temp = _draft_temperature_for_attempt(
                            base_temp=float(args.temperature),
                            jitter=max(0.0, float(args.draft_temperature_jitter)),
                            attempt_idx=attempt_idx,
                        )
                        draft_temp = max(0.0, min(1.5, float(draft_temp)))
                        generation_temperature = draft_temp
                        selected_attempt = attempt_idx + 1
                        draft_gen = generate_ollama_response(
                            host=args.host,
                            model=arm.model,
                            prompt=prompt,
                            temperature=draft_temp,
                            max_tokens=max(1, int(args.draft_max_tokens)),
                            timeout_s=max(1, min(int(args.timeout_s), int(args.draft_timeout_s))),
                        )
                        draft_total_eval_count += int(draft_gen.get("eval_count", 0) or 0)
                        draft_total_duration_ns += int(draft_gen.get("total_duration_ns", 0) or 0)

                        if not bool(draft_gen.get("ok")):
                            draft_errors += 1
                            draft_last_error = str(draft_gen.get("error", ""))
                            continue

                        draft_text = str(draft_gen.get("response", ""))
                        draft_score = draft_candidate_score(
                            draft_text,
                        ...
                        draft_last_score = float(draft_score)
                        if math.isnan(draft_best_score) or draft_score > draft_best_score:
                            draft_best_score = float(draft_score)

                        if draft_score >= draft_threshold:
                            draft_accepted = True
                            accepted_attempt = attempt_idx + 1
                            break
                        draft_rejected_attempts += 1

                    draft_meta = {
                        "draft_prefilter_enabled": True,
                        "draft_attempts": selected_attempt,
                        "draft_rejected_attempts": draft_rejected_attempts,
                        "draft_accepted": bool(draft_accepted),
                        "draft_selected_attempt": selected_attempt,
                        "draft_accepted_attempt": accepted_attempt,
                        "draft_score_threshold": float(draft_threshold),
                        "draft_last_score": draft_last_score,
                        "draft_best_score": draft_best_score,
                        "draft_selected_temperature": float(generation_temperature),
                        "draft_total_duration_ns": int(draft_total_duration_ns),
                        "draft_eval_count": int(draft_total_eval_count),
                        "draft_error_count": int(draft_errors),
                        "draft_last_error": draft_last_error,
                    }

                if arm.generation_backend == "gspe_local_hf":
                    gspe_engine = gspe_engine_by_arm.get(arm.arm_id)
                    if gspe_engine is None:
                        gen = {"ok": False, "error": f"missing_gspe_engine_for_arm:{arm.arm_id}"}
                    else:
                        gspe_state = build_gspe_game_state(
                            scenario=scenario,
                            scenario_tags=scenario_tags,
                            dynamic_context=scenario_context,
                        )
                        try:
                            gspe_result = gspe_engine.generate(
                                game_state=gspe_state,
                                persona=scenario_persona,
                                player_input=scenario_input,
                                retrieved_passages=[
                                    str(item)
                                    for item in (scenario.get("retrieved_passages") or [])[:3]
                                    if str(item).strip()
                                ],
                            )
                            total_ms = float(gspe_result.get("total_ms", float("nan")))
                            ttft_ms = float(gspe_result.get("ttft_ms", float("nan")))
                            eval_ns = 0
                            total_ns = 0
                            if not math.isnan(total_ms):
                                total_ns = int(max(0.0, total_ms) * 1_000_000.0)
                            if not math.isnan(total_ms) and not math.isnan(ttft_ms):
                                eval_ns = int(max(0.0, total_ms - ttft_ms) * 1_000_000.0)
                            gen = {
                                "ok": True,
                                "response": str(gspe_result.get("response", "")),
                                "eval_count": 0,
                                "prompt_eval_count": 0,
                                "eval_duration_ns": eval_ns,
                                "total_duration_ns": total_ns,
                                "ttft_ms": ttft_ms,
                                "prefix_cache_hit_rate": float(gspe_result.get("prefix_cache_hit_rate", float("nan"))),
                            }
                        except Exception as exc:
                            gen = {"ok": False, "error": f"gspe_generation_failed:{exc}"}
                else:
                ...
                record = {
                    ...
                    "episodic_memory_enabled": bool(episodic_store is not None),
                    "episodic_memory_retrieved": int(len(episodic_memories)),
                    "episodic_memory_top_score": (
                        float(episodic_memories[0].get("score", float("nan")))
                        if episodic_memories
                        else float("nan")
                    ),
                }
                record.update(gen)
                record.update(draft_meta)
                if behavior_state:
                ...
                if episodic_store is not None:
                    final_response = str(record.get("response", "")).strip()
                    if bool(record.get("ok")) and final_response:
                        episodic_store.add_record(
                            npc_id=scenario_persona,
                            persona=scenario_persona,
                            behavior_state=behavior_state,
                            location=location,
                            ...
                            tags=[
                                str(scenario_tags.get("conflict_type", "")).strip(),
                                str(scenario_tags.get("persona_archetype", "")).strip(),
                                str(scenario_tags.get("location_type", "")).strip(),
                            ],
                            source="proposal_eval",
                            run_id=run_id,
                        )
                        record["episodic_memory_written"] = True
                    else:
                        record["episodic_memory_written"] = False
    ...
    if episodic_store_by_arm:
        episodic_summary: Dict[str, Any] = {"enabled": True, "stores": {}}
        for arm_id, store in episodic_store_by_arm.items():
            episodic_summary["stores"][arm_id] = store.stats()
        write_json(run_dir / "episodic_memory_summary.json", episodic_summary)
    ...
    gspe_arm_ids = [arm.arm_id for arm in arms if arm.generation_backend == "gspe_local_hf"]
    if gspe_arm_ids and "proposed_contextual_controlled" in available_arm_ids:
        for gspe_id in gspe_arm_ids:
            comparison_plan.append(
                (f"{gspe_id}_vs_proposed_contextual_controlled", gspe_id, "proposed_contextual_controlled")
            )
