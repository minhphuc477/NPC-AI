...
def latest_run_dir(root: Path) -> Path:
    ...
    dirs = [
        ...
        and p.name != "latest"
        and not p.name.endswith("_batch_tmp")
    ]
    if not dirs:
...
def artifact_root(subdir: str) -> Path:
    rel = Path(subdir)
    preferred = STORAGE_ARTIFACT_ROOT / rel
    legacy = LEGACY_ARTIFACT_ROOT / rel
...
def resolve_run_path(raw: str, default_root: Path, fallback_root: Optional[Path] = None) -> Path:
    ...
    if token.lower() == "latest":
        try:
            ...
            if fallback_root is not None:
                return latest_run_dir(fallback_root)
            raise
    path = Path(token)
    if path.exists():
        return path
    ...
    if candidate.exists():
        return candidate
    if fallback_root is not None:
        fallback_candidate = fallback_root / token
...
def main() -> None:
    ...
    parser.add_argument("--proposal-root", default="storage/artifacts/proposal", help="Proposal artifacts root")
    parser.add_argument("--publication-root", default="storage/artifacts/publication", help="Publication artifacts root")
    ...
    proposal_fallback = artifact_root("proposal") if proposal_root != artifact_root("proposal") else None
    publication_fallback = (
        artifact_root("publication")
        if publication_root != artifact_root("publication")
        else None
    )
    proposal_run = resolve_run_path(str(args.proposal_run), proposal_root, proposal_fallback)
    publication_run = (
        None
        if args.skip_publication
        else resolve_run_path(str(args.publication_run), publication_root, publication_fallback)
    )
