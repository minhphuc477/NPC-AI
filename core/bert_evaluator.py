...
class BERTSemanticEvaluator:
    ...
    def calculate_similarity(
        ...
        cosine_similarity = getattr(pairwise, "cosine_similarity")
    ...
    def detect_emotion(self, text: str) -> Tuple[str, float]:
        ...
        results = [cast(Dict[str, Any], r) for r in raw_results if isinstance(r, dict)]
        
        # Get top emotion
        top_emotion = max(results, key=lambda x: float(x.get("score", 0.0)))
        label = str(top_emotion.get("label", "neutral"))
    ...
    def evaluate_response(
        ...
        if len(sentences) > 1:
            ...
            scores['coherence'] = float(np.mean(coherence_scores)) if coherence_scores else 0.5
        else:
    ...
    def fine_tune_for_game_dialogue(
        ...
        try:
            st_module = importlib.import_module("sentence_transformers")
            InputExample = getattr(st_module, "InputExample")
        ...
        train_dataset = cast(Any, examples)
        dataloader = DataLoader(train_dataset, shuffle=True, batch_size=max(1, batch_size))
